...
- **每日诊断**：每次启动自动触发 30 题全域诊断，持续更新薄弱点数据，考后给出最高效的学习建议；所有域正确率达标后自动关闭
...
├── modes/
│   ├── daily_diagnostic.py      # 每日诊断：30题全域 + 薄弱点推荐
│   ├── study_mode.py            # 学习模式（缓存优先）
│   ├── practice_mode.py         # 练习模式
│   ├── exam_mode.py             # 模拟考试（CAT）
│   └── review_mode.py           # 错题复习
...
每次运行 `python main.py` 时自动触发，构建「诊断 → 学习 → 再诊断」正向循环：

```
启动
 ├─ 全部达标（各域答题 ≥ 100 次 + 正确率 ≥ 70%）→ 提示达标，不再触发
 ├─ 今日已完成 → 跳过
 ├─ [s] 跳过 → 进入主菜单
 └─ [Enter] 开始
      30 题（按考试权重分配，混合难度）
      → 答题结果写入 answer_records + weakness_records
      → 显示本次得分 & 各域正确率
      → 推荐 Top 3 最值得学习的域/子域
           紧迫度 = (1 - 正确率) × 域权重 + 子域薄弱惩罚
      → 进入主菜单
```
